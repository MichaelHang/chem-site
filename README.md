...
- 密码任意；前端只传 cyrb53 派生值，服务器 scrypt 加盐哈希存储，Bearer token 有效期 30 天
- **token 过期后自动登出**：过期后的首次同步收到 401，前端自动清除本地会话并弹出登录框提示「登录已过期」；云端数据不受影响，重新登录即恢复同步
- 登录未知用户名会**自动注册**（首次输入即建账户），输错用户名会意外创建新号

## 已知限制

- 同步冲突以「客户端时间戳 vs 服务器时间戳」判定，客户端时钟明显偏慢时可能被拒收（客户端已串行化推送降低触发概率）
- 服务端 `sessions` 表过期 token 不清理，长期运行建议定期清表
